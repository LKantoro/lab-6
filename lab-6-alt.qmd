...
summarize_weather = function(df = nycflights13::flights, num_var){
  ...
  #if (class(df %>% select({{num_var}})) != 'numeric'){
...
#testing with data frame not similar to flights
summarize_weather(mpg, dep_time)


df %>%
    select(dep_time) %>%
    map_lgl(~ (is.character(dep_time) | is.factor(dep_time))) %>%
    keep(identity) %>%
    names()
...
'carrier' %in% nycflights13::flights %>% purrr::discard(~!is.numeric(.)) %>% names
