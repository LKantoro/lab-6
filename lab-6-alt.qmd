...
```{r}
#| label: question-12
sorted_bars(mpg, model)

ggplot2::mpg %>% purrr::discard(~is.numeric(.)) %>% names
...
```{r}
mpg %>% 
    mutate(across(c(year, cyl), ~map()))

map(sorted_bars(mpg, c(manufacturer, model)))




mpg %>% select(where(is.character)) %>% map(~sorted_bars(df = mpg, .x))



mpg %>% select(model) %>% map(~sorted_bars(df = mpg, var = .x))




map(mpg %>% select(model), \(x) sorted_bars(df = mpg, var == {{ x }}))

mpg %>% select(where(is.character)) %>% map(~sorted_bars(df = mpg, var = .x))
...
```{r}
#https://stackoverflow.com/questions/76807848/use-map-function-to-create-multiple-plots-in-r
