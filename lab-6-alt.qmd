...
summarize_severe = function(df = nycflights13::flights){
  ...
  return(filter_severe(df) %>% 
           summarise(Cancelled = sum(is.na(arr_time)),
                                         Delayed = sum(dep_time > 60, na.rm = TRUE)))
...
sorted_bars <- function(df, var, name = englue("{{ var }}")) {
  ...
  return(df %>% mutate({{var}} := fct_infreq({{var}})) %>% 
    ggplot(aes(x = {{var}})) + ggtitle(paste("Distribution of", {name}, "(decreasing)")) + 
    geom_bar())
...
df %>% 
    mutate(across(c(year, cyl), ~ replace_na(data = .x, 
                                       replace = 5)))
```
